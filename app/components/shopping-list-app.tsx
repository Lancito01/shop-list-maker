...
import {
    closestCenter, DndContext, type DragEndEvent, MouseSensor, TouchSensor, useSensor, useSensors,
} from "@dnd-kit/core";
import {arrayMove, SortableContext, useSortable, verticalListSortingStrategy,} from "@dnd-kit/sortable";
import {CSS} from "@dnd-kit/utilities";

import {
    displayCurrencies, type DisplayCurrency, entryCurrencies, type EntryCurrency, formatCurrency, toNumber,
} from "@/lib/currency";
import type {ListType, ShoppingItemRecord, ShoppingListRecord,} from "@/lib/data/shopping";
import {convertAmount, type ExchangeRatesSnapshot,} from "@/lib/exchange-rates";

type ShoppingListResponse = {
    lists: ShoppingListRecord[]; error?: string;
};
...
type EditDraft = {
    itemId: string; name: string; quantity: string; absPrice: string; isNeg: boolean; currency: EntryCurrency;
};

type ItemUpdateInput = {
    name: string; quantity: string; unitPrice?: string; currency: EntryCurrency; completed?: boolean;
};

const defaultNewItem = {
    name: "", quantity: "1", unitPrice: "", currency: "USD" as EntryCurrency,
};
...
function DragHandleIcon() {
    ...
    >
        <path
            d="M7 4a1 1 0 11-2 0 1 1 0 012 0zm0 6a1 1 0 11-2 0 1 1 0 012 0zm-1 7a1 1 0 100-2 1 1 0 000 2zm8-13a1 1 0 11-2 0 1 1 0 012 0zm0 6a1 1 0 11-2 0 1 1 0 012 0zm-1 7a1 1 0 100-2 1 1 0 000 2z"/>
    </svg>);
}
...
type SortableRowProps = {
    id: string; disabled?: boolean; children: (drag: {
...
function SortableRow({id, disabled, children}: SortableRowProps) {
    ...
    const style = {
        transform: CSS.Transform.toString(transform), transition, zIndex: isDragging ? 2 : 0,
    };
...
export function ShoppingListApp() {
    ...
    const loadLists = useCallback(async () => {
        ...
        try {
            ...
            setEditingDraft((current) => {
                ...
                const stillExists = payload.lists.some((list) => list.items.some((item) => item.id === current.itemId),);
            ...
            setTodoEditItemId((current) => {
                ...
                const stillExists = payload.lists.some((list) => list.items.some((item) => item.id === current),);
                if (!stillExists) {
                    setTodoEditName("");
                }
                return stillExists ? current : null;
    ...
    const selectedListItemIds = useMemo(() => selectedList?.items.map((item) => item.id) ?? [], [selectedList],);

    const selectedListHasEntries = (selectedList?.items.length ?? 0) > 0;
    const selectedListAllEntriesChecked = useMemo(() => Boolean(selectedListHasEntries && selectedList?.items.every((item) => item.completed)), [selectedList, selectedListHasEntries],);

    const selectedListEntrySignature = useMemo(() => selectedList?.items
        ...
        resetFrameId = window.requestAnimationFrame(() => {
            ...
            measureFrameId = window.requestAnimationFrame(() => {
                ...
                for (const itemId of itemIds) {
                    ...
                    nextTallModeById[itemId] = titleEl ? titleEl.scrollWidth > titleEl.clientWidth + 1 : false;
                }
    ...
    }, [selectedListId, selectedListItemIds, selectedListEntrySignature, entryMeasureTick, editingDraft?.itemId, todoEditItemId,]);

    const dndSensors = useSensors(useSensor(MouseSensor, {activationConstraint: {distance: 8}}), useSensor(TouchSensor, {
        activationConstraint: {delay: 220, tolerance: 8},
    }),);

    const dragDisabled = reorderingItems || savingItem || bulkUpdatingEntries || Boolean(editingDraft) || Boolean(todoEditItemId);
    const listDragDisabled = reorderingLists || creatingList || loadingLists || refreshingLists;
    const checkAllEntriesDisabled = !selectedListHasEntries || bulkUpdatingEntries || creatingItem || savingItem || reorderingItems || Boolean(editingDraft) || Boolean(todoEditItemId);

    const convertSubtotal = useCallback((subtotal: number, entryCurrency: EntryCurrency): number | null => {
    ...
    const hasEstimatedItems = useMemo(() => selectedList?.type === "budget" ? selectedList.items.some((item) => !item.unitPrice) : false, [selectedList],);
    ...
    const budgetAppliedSubtotalsByItemId = useMemo(() => {
        const appliedSubtotals = new Map<string, {
        ...
        type IncomeSource = {
            itemId: string; remaining: number;
        };
        ...
        for (const item of selectedList.items) {
            if (!item.unitPrice) {
                appliedSubtotals.set(item.id, {
                    appliedPreferred: null, originalPreferred: null,
                });
                continue;
            }
            ...
            if (subtotalPreferred < 0) {
                if (item.completed) {
                    incomeSources.push({
                        itemId: item.id, remaining: Math.abs(subtotalPreferred),
                    });
                }

                appliedSubtotals.set(item.id, {
                    appliedPreferred: item.completed ? subtotalPreferred : 0, originalPreferred: subtotalPreferred,
                });
                continue;
            }

            appliedSubtotals.set(item.id, {
                appliedPreferred: 0, originalPreferred: subtotalPreferred,
            });
        }
        ...
        for (const item of selectedList.items) {
            ...
            appliedSubtotals.set(item.id, {
                appliedPreferred: subtotalPreferred - remainingExpense, originalPreferred: subtotalPreferred,
            });
        }

        for (const source of incomeSources) {
            const current = appliedSubtotals.get(source.itemId);
            appliedSubtotals.set(source.itemId, {
    ...
    async function setAllEntriesCompleted(completed: boolean) {
        ...
        const entriesToUpdate = selectedList.items.filter((item) => item.completed !== completed,);
        ...
        try {
            await Promise.all(entriesToUpdate.map((item) => patchItem(item.id, {
                name: item.name,
                quantity: item.quantity,
                unitPrice: item.unitPrice ?? undefined,
                currency: item.currency,
            ...
            const message = error instanceof Error ? error.message : "Unable to update all entries.";
            setErrorMessage(message);
            await loadLists();
        } finally {
            setUpdatingItems((current) => {
    ...
    async function saveTodoEntry(item: ShoppingItemRecord) {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : "Unable to save todo entry.";
            setErrorMessage(message);
        } finally {
            setUpdatingItems((current) => ({...current, [item.id]: false}));
        }
    ...
    async function saveEditedItem() {
        ...
        const editingItem = selectedList?.items.find((item) => item.id === editingDraft.itemId,);

        const trimmedPrice = editingDraft.absPrice.trim();
        const finalPrice = trimmedPrice === "" ? undefined : editingDraft.isNeg ? `-${trimmedPrice}` : trimmedPrice;
    ...
    async function addItem() {
        ...
        const finalPrice = normalizedUnitPrice === "" ? undefined : newItemPriceNegative ? `-${normalizedUnitPrice}` : normalizedUnitPrice;
        ...
        try {
            const response = await fetch("/api/items", {
                method: "POST", headers: {
                    ...
                    completed: newItemCompleted, ...(finalPrice === undefined ? {} : {unitPrice: finalPrice}),
                }),
            });
            const payload = (await response.json()) as MutationErrorResponse;
            if (!response.ok) {
    ...
    async function persistReorderedItems(listId: string, itemIds: string[]) {
        const response = await fetch("/api/items/reorder", {
            method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({listId, itemIds}),
        });
        const payload = (await response.json()) as MutationErrorResponse;
        if (!response.ok) {
    ...
    async function persistReorderedLists(listIds: string[]) {
        const response = await fetch("/api/lists/reorder", {
            method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({listIds}),
        });
        const payload = (await response.json()) as MutationErrorResponse;
        if (!response.ok) {
    ...
    async function handleEntriesDragEnd(event: DragEndEvent) {
        ...
        const reorderedItems = arrayMove(selectedList.items, oldIndex, newIndex).map((item, index) => ({
            ...item, sortOrder: index,
        }),);

        setLists((currentLists) => currentLists.map((list) => list.id === selectedList.id ? {
        ...
        try {
            await persistReorderedItems(selectedList.id, reorderedItems.map((item) => item.id),);
        } catch (error) {
            const message = error instanceof Error ? error.message : "Unable to reorder entries.";
            setErrorMessage(message);
            await loadLists();
        } finally {
        ...
        try {
            const response = await fetch("/api/items", {
                method: "POST", headers: {
                    ...
                    listId: selectedListId, name: trimmedName, quantity: "1", currency: "USD", completed: false,
                }),
            });
            const payload = (await response.json()) as MutationErrorResponse;
            if (!response.ok) {
            ...
            const message = error instanceof Error ? error.message : "Unable to add todo entry.";
            setErrorMessage(message);
        } finally {
            setCreatingItem(false);
        }
    ...
    return (<div className="flex flex-col gap-6">
        <aside
            className="w-full rounded-2xl border border-white/10 bg-zinc-900/70 p-4 shadow-2xl shadow-black/30 backdrop-blur">
            <div>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <label className="flex w-full items-center gap-2 text-xs text-zinc-400 sm:w-auto sm:text-sm">
                        ...
                        >
                            {displayCurrencies.map((currency) => (<option key={currency} value={currency}>
                                {currency}
                            </option>))}
                        </select>
                    </label>
                ...
                >
                    <SortableContext items={listIds} strategy={verticalListSortingStrategy}>
                        {lists.map((list) => {
                            ...
                            >
                                {({attributes, listeners, isDragging}) => (<div
                                    role="button"
                                    tabIndex={0}
                                    onClick={() => {
                                    ...
                                    className={`flex cursor-pointer items-start justify-between rounded-md border px-3 py-2 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/60 sm:items-center ${active ? "border-cyan-400/50 bg-zinc-800/80 shadow-lg shadow-cyan-950/30" : "border-white/10 bg-zinc-900/70 hover:bg-zinc-800/80"} ${isDragging ? "ring-1 ring-cyan-400/40 opacity-90" : ""}`}
                                >
                                    <div className="mr-2">
                                        <button
                                            type="button"
                                    ...
                                    <div className="min-w-0 flex-1 cursor-pointer">
                                        ...
                                        {list.type === "budget" ? (
                                            <p className="mt-0.5 whitespace-pre-line break-words text-left text-xs leading-snug text-zinc-500 sm:text-sm">
                                                Total:{" "}
                                                {listTotalsById.get(list.id)?.unavailable ? "Rate unavailable" : formatCurrency(listTotalsById.get(list.id)?.total ?? 0, preferredCurrency,)}
                                            </p>) : (
                                            <p className="mt-0.5 whitespace-pre-line break-words text-left text-xs leading-snug text-zinc-500 sm:text-sm">
                                                Todo list
                                            </p>)}
        ...
        <section
            className="rounded-2xl border border-white/10 bg-zinc-900/70 p-4 shadow-2xl shadow-black/30 backdrop-blur">
            {!selectedList && (<p className="text-sm text-zinc-400">
                Select or create a list to start adding entries.
            </p>)}

            {selectedList && (<div className="space-y-4">
                {/* Budget header */}
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <h2 className="text-xl font-semibold text-zinc-100">{selectedList.name}</h2>
                    {selectedList.type === "budget" ? (<div className="flex flex-wrap items-center justify-end gap-2">
                        <p
                            className={`rounded-full border px-3 py-1 text-sm font-semibold ${selectedListTotals.unavailable ? "border-rose-400/40 bg-rose-500/10 text-rose-300" : hasEstimatedItems ? "border-amber-400/50 bg-amber-500/10 text-amber-200" : -selectedListTotals.total > 0 ? "border-emerald-400/50 bg-emerald-500/20 text-emerald-200" : -selectedListTotals.total < 0 ? "border-rose-400/40 bg-rose-500/10 text-rose-300" : "border-cyan-400/40 bg-cyan-500/10 text-cyan-200"}`}
                        >
                            {selectedListTotals.unavailable ? "All entries unavailable (exchange rates needed)" : `${hasEstimatedItems ? "Est. " : ""}All entries: ${formatCurrency(-selectedListTotals.total, preferredCurrency)}`}
                            {!selectedListTotals.unavailable && hasEstimatedItems && (
                                <span className="ml-1.5 text-xs opacity-70">(some prices TBD)</span>)}
                        </p>
                        <p
                            className={`rounded-full border px-3 py-1 text-sm font-semibold ${selectedEntriesTotals.unavailable ? "border-rose-400/40 bg-rose-500/10 text-rose-300" : !selectedEntriesTotals.hasAny ? "border-white/15 bg-zinc-800/60 text-zinc-300" : selectedEntriesTotals.estimated ? "border-amber-400/50 bg-amber-500/10 text-amber-200" : selectedEntriesTotals.total > 0 ? "border-emerald-400/50 bg-emerald-500/20 text-emerald-200" : selectedEntriesTotals.total < 0 ? "border-rose-400/40 bg-rose-500/10 text-rose-300" : "border-cyan-400/40 bg-cyan-500/10 text-cyan-200"}`}
                        >
                            {selectedEntriesTotals.unavailable ? "Selected unavailable (exchange rates needed)" : `Selected: ${formatCurrency(selectedEntriesTotals.total, preferredCurrency)}`}
                            {!selectedEntriesTotals.unavailable && !selectedEntriesTotals.hasAny && (
                                <span className="ml-1.5 text-xs opacity-70">(none checked)</span>)}
                            {!selectedEntriesTotals.unavailable && selectedEntriesTotals.hasAny && selectedEntriesTotals.estimated && (
                ...
                {selectedList.type === "budget" && (exchangeRatesLoading || exchangeRatesError) && (
                    <p className="text-xs text-zinc-500">
                        {exchangeRatesLoading ? "Refreshing exchange rates..." : `Exchange rates unavailable: ${exchangeRatesError}`}
                    </p>)}
                ...
                {selectedList.type === "budget" ? (<>
                    ...
                    <div className="rounded-xl border border-white/10 bg-zinc-950/60 p-3 space-y-2">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            />
                            <label
                                className="flex w-full cursor-pointer select-none items-center gap-2 whitespace-nowrap text-sm text-zinc-400 sm:w-auto">
                                <input
                                    type="checkbox"
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            ...
                            <div className="flex min-w-0 w-full items-center gap-1.5 sm:flex-1">
                                <button
                                    ...
                                    className={`shrink-0 rounded-xl border px-3 py-2.5 text-sm font-bold transition ${newItemPriceNegative ? "border-emerald-400/50 bg-emerald-500/20 text-emerald-200" : "border-white/15 bg-zinc-800/60 text-zinc-300 hover:bg-zinc-700"}`}
                                >
                                ...
                                <select
                                    ...
                                    onChange={(event) => setNewItem((current) => ({
                                        ...current, currency: event.target.value as EntryCurrency,
                                    }))}
                                ...
                                >
                                    {entryCurrencies.map((currency) => (<option key={currency} value={currency}>
                                        {currency}
                                    </option>))}
                                </select>
                            </div>
                    ...
                    <div className="space-y-1.5">
                        {selectedList.items.length === 0 && (<p className="py-4 text-center text-sm text-zinc-400">
                        ...
                        {selectedList.items.length > 0 && (<>
                            <div className="flex items-center justify-end px-1">
                                ...
                                >
                                    {bulkUpdatingEntries ? "Updating..." : selectedListAllEntriesChecked ? "Uncheck all" : "Check all"}
                                </button>
                            </div>
                            <div
                                className="hidden md:grid md:grid-cols-[2rem_2rem_minmax(0,1fr)_8.5rem_8.5rem_11rem] items-center gap-3 px-3 text-[11px] font-semibold uppercase tracking-wide text-zinc-500">
                            ...
                            >
                                ...
                                >
                                    {selectedList.items.map((item) => {
                                        ...
                                        const appliedSubtotalPreferred = subtotalSummary?.appliedPreferred ?? (isTbd ? null : convertSubtotal(subtotalOriginal, item.currency));
                                        const originalSubtotalPreferred = subtotalSummary?.originalPreferred ?? (isTbd ? null : convertSubtotal(subtotalOriginal, item.currency));
                                        ...
                                        const isConversionUnavailable = !isTbd && originalSubtotalPreferred == null;
                                        const isSourceDepleted = isIncome && appliedSubtotalPreferred != null && Math.abs(appliedSubtotalPreferred) <= epsilon;
                                        const isPendingExpense = !isTbd && !isConversionUnavailable && !isIncome && item.completed && appliedSubtotalPreferred != null && originalSubtotalPreferred != null && originalSubtotalPreferred - appliedSubtotalPreferred > epsilon;

                                        const rowTheme = isTbd ? "border-amber-400/30 bg-amber-500/5" : isConversionUnavailable ? item.completed ? "border-rose-400/35 bg-rose-500/10" : "border-rose-400/20 bg-zinc-900/60" : isIncome ? item.completed ? isSourceDepleted ? "border-emerald-500/30 bg-emerald-500/5" : "border-emerald-400/40 bg-emerald-500/15" : "border-emerald-500/20 bg-zinc-900/60" : item.completed ? isPendingExpense ? "border-amber-400/35 bg-amber-500/10" : "border-white/10 bg-zinc-950/40" : "border-white/10 bg-zinc-900/70";

                                        const appliedSubtotalColor = isTbd ? "text-amber-300" : isConversionUnavailable ? "text-rose-300" : isIncome ? item.completed ? isSourceDepleted ? "text-emerald-200/75" : "text-emerald-300" : "text-emerald-300/60" : item.completed ? isPendingExpense ? "text-amber-200" : "text-zinc-100" : "text-zinc-500";

                                        const originalSubtotalColor = isTbd ? "text-amber-300/70" : isIncome ? "text-emerald-200/70" : "text-zinc-500";

                                        const entryTitleColor = isTbd ? "text-zinc-100" : isIncome ? item.completed ? "text-zinc-100" : "text-emerald-200/65" : item.completed ? "text-zinc-100" : "text-zinc-500";

                                        const qtyPriceColor = !isTbd && !isIncome && !item.completed ? "text-zinc-500" : "text-zinc-400";

                                        const expenseRemainingSubtotalPreferred = !isTbd && !isConversionUnavailable && !isIncome && appliedSubtotalPreferred != null && originalSubtotalPreferred != null ? Math.max(originalSubtotalPreferred - appliedSubtotalPreferred, 0,) : null;
                                        const appliedSubtotalLabel = isTbd ? "TBD" : isConversionUnavailable ? "Rates" : formatCurrency(expenseRemainingSubtotalPreferred ?? appliedSubtotalPreferred ?? 0, preferredCurrency,);
                                        const originalSubtotalLabel = isTbd ? "TBD" : formatCurrency(subtotalOriginal, item.currency);
                                        ...
                                        >
                                            {({attributes, listeners, isDragging}) => {
                                                if (isThisEditing && editingDraft) {
                                                    ...
                                                    >
                                                        <input
                                                            ...
                                                            onChange={(e) => setEditingDraft((d) => d ? {
                                                        ...
                                                        <div
                                                            ...
                                                            <div
                                                                className="flex min-w-0 w-full items-center gap-1 sm:w-auto sm:flex-1">
                                                                <button
                                                                    type="button"
                                                                    onClick={() => setEditingDraft((d) => d ? {
                                                                        ...d, isNeg: !d.isNeg
                                                                    } : d,)}
                                                                    title={editingDraft.isNeg ? "Currently: income (−). Click to switch to expense (+)." : "Currently: expense (+). Click to switch to income (−)."}
                                                                    className={`rounded-lg border px-2.5 py-2 text-sm font-bold transition ${editingDraft.isNeg ? "border-emerald-400/50 bg-emerald-500/20 text-emerald-200" : "border-white/15 bg-zinc-800/60 text-zinc-300 hover:bg-zinc-700"}`}
                                                                >
                                                                    {editingDraft.isNeg ? "−" : "+"}
                                                                </button>
                                                                <input
                                                                ...
                                                                <select
                                                                    ...
                                                                    onChange={(e) => setEditingDraft((d) => d ? {
                                                                        ...d, currency: e.target.value as EntryCurrency,
                                                                    } : d,)}
                                                            ...
                                                            <div
                                                                className="ml-auto flex w-full justify-end gap-2 sm:w-auto">
                                                                <button
                                                                    type="button"
                                                ...
                                                const priceLabel = isTbd ? "TBD" : formatCurrency(Math.abs(toNumber(item.unitPrice)), item.currency);
                                                const completionLabel = isIncome ? `Use ${item.name} as source` : `Mark ${item.name} as completed`;

                                                const completionCheckbox = (<input
                                                    type="checkbox"
                                                    checked={item.completed}
                                                    onChange={(e) => void toggleItemCompleted(item, e.target.checked)}
                                                    disabled={isDeleting || isUpdating}
                                                    className={`h-4 w-4 rounded disabled:cursor-not-allowed disabled:opacity-60 md:mx-auto ${isIncome ? "accent-emerald-400" : "accent-cyan-400"}`}
                                                    aria-label={completionLabel}
                                                />);

                                                const subtotalDisplay = (<span
                                                    className="flex shrink-0 flex-col whitespace-nowrap text-right tabular-nums md:items-end">
                                                    <span
                                                        className={`text-sm font-semibold ${appliedSubtotalColor}`}>
                                                ...
                                                if (isTallMode) {
                                                    return (<div
                                                        className={`flex flex-col gap-2 rounded-xl border px-3 py-2.5 md:grid md:grid-cols-[2rem_2rem_minmax(0,1fr)_8.5rem_8.5rem_11rem] md:items-center md:gap-3 ${rowTheme} ${isDragging ? "ring-1 ring-cyan-400/40 opacity-90" : ""}`}
                                                    >
                                                        <div
                                                            className="flex min-w-0 items-center gap-3 md:contents">
                                                            {completionCheckbox}
                                                            <button
                                                                type="button"
                                                                {...attributes}
                                            ...
                                            {item.name || (<span className="italic text-zinc-500">Unnamed entry</span>)}
                                                        ...
                                                        <div
                                                            className="flex min-w-0 items-center justify-between gap-2 pl-[3.25rem] md:contents md:pl-0">
                                                            <span
                                                ...
                                                return (<div
                                                    className={`flex items-center gap-3 rounded-xl border px-3 py-2.5 md:grid md:grid-cols-[2rem_2rem_minmax(0,1fr)_8.5rem_8.5rem_11rem] md:gap-3 ${rowTheme} ${isDragging ? "ring-1 ring-cyan-400/40 opacity-90" : ""}`}
                                                >
                                                    {completionCheckbox}
                                                    <button
                                                        type="button"
                                        ...
                                        {item.name || (<span className="italic text-zinc-500">Unnamed entry</span>)}
                                                    ...
                                                    <span
                                                        className={`hidden whitespace-nowrap text-right text-xs tabular-nums md:block ${qtyPriceColor}`}>
                                        {qtyNum !== 1 ? `${item.quantity} × ${priceLabel}` : `${priceLabel}`}
                                      </span>
                                                    ...
                                                    <div
                                                        className="ml-auto flex shrink-0 items-center justify-end gap-1 md:ml-0">
                                                        {editButton}
                                                        {deleteButton}
                                                    </div>
                                                </div>);
                    ...
                    <div className="space-y-1.5">
                        ...
                        {selectedList.items.length > 0 && (<DndContext
                            sensors={dndSensors}
                            collisionDetection={closestCenter}
                            onDragEnd={(event) => {
                                void handleEntriesDragEnd(event);
                                ...
                                {selectedList.items.map((item) => {
                                    ...
                                    >
                                        {({attributes, listeners, isDragging}) => {
                                            if (isEditing) {
                                                return (<div
                                                    className="flex min-w-0 flex-wrap items-center gap-2 rounded-xl border border-white/10 bg-zinc-900/70 px-3 py-2.5">
                                                    <input
                                                        type="checkbox"
                                                        checked={item.completed}
                                                        onChange={(e) => void toggleItemCompleted(item, e.target.checked)}
                                            ...
                                            if (isTallMode) {
                                                return (<div
                                                    className={`flex flex-col gap-2 rounded-xl border px-3 py-2.5 md:grid md:grid-cols-[2rem_2rem_minmax(0,1fr)_11rem] md:items-center md:gap-3 ${item.completed ? "border-white/10 bg-zinc-900/70" : "border-white/10 bg-zinc-950/40"} ${isDragging ? "ring-1 ring-cyan-400/40 opacity-90" : ""}`}
                                                >
                                                    <div
                                                        className="flex min-w-0 items-center gap-3 md:contents">
                                                        <input
                                                        ...
                                                        <span
                                                            ref={(node) => setEntryTitleRef(item.id, node)}
                                                            className={`min-w-0 flex-1 break-words text-sm leading-snug md:flex-none md:leading-normal ${item.completed ? "text-zinc-500 line-through" : "text-zinc-100"}`}
                                                        >
                                          {item.name || (<span className="italic text-zinc-500">Unnamed entry</span>)}
                                        </span>
                                                    </div>
                                                    <div
                                                        className="pl-[3.25rem] md:contents md:pl-0">
                                                        <div
                                            ...
                                            return (<div
                                                className={`flex items-center gap-3 rounded-xl border px-3 py-2.5 md:grid md:grid-cols-[2rem_2rem_minmax(0,1fr)_11rem] md:items-center md:gap-3 ${item.completed ? "border-white/10 bg-zinc-900/70" : "border-white/10 bg-zinc-950/40"} ${isDragging ? "ring-1 ring-cyan-400/40 opacity-90" : ""}`}
                                            >
                                                <input
                                                    type="checkbox"
                                                    checked={item.completed}
                                                ...
                                                <span
                                                    ref={(node) => setEntryTitleRef(item.id, node)}
                                                    className={`min-w-0 flex-1 truncate text-sm md:flex-none ${item.completed ? "text-zinc-500 line-through" : "text-zinc-100"}`}
                                                >
                                      {item.name}
                                    </span>
                                                <div
